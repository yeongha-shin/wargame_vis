...
> (현재 기본값: `results/simulation.csv`, `results/cameras.json`). 파일을 바꾸면 새로고침만 하면 됩니다.
...
- 지형 PNG는 고정된 120 m × 120 m 게임 평면 위에 입히는 스킨일 뿐 — 유닛 위치(`results/simulation.csv`)는 자동으로 새 좌표로 이동하지 않습니다.
